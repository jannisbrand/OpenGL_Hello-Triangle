...
int main()
{
    ...
    Shader textureShader("../shaders/vertex_texture_exercise-1.vert", "../shaders/fragment_texture_exercise-4.frag");
    textureShader.use();
    textureShader.setInt("textureSampler1", 0);
    textureShader.setInt("textureSampler2", 1);

    float textureBlend = 0.2;
    textureShader.setFloat("texBlend", textureBlend);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (glfwGetKey(window, GLFW_KEY_UP) && textureBlend < 1)
        {
            textureBlend += 0.0001;
        }
        else if (glfwGetKey(window, GLFW_KEY_DOWN) && textureBlend > 0)
        {
            textureBlend -= 0.0001;
        }
        std::cout << textureBlend << std::endl;

        glUniform1f(glGetUniformLocation(textureShader.ID, "textureBlend"), textureBlend);
        // textureShader.setFloat("textureBlend", textureBlend);
        textureShader.use();
