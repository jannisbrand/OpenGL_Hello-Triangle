...
int main()
{
    ...
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void*) 0);  // Vertex position (x,y,z)
    // --> No vertex color data in the base example! (1)
    glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void*) (3 * sizeof(float)));    // Texture coordinates (x,y)
    ...
    // ***********************************SHADER****************************************
    Shader baseShader("../shaders/vertex_lighting_base.vert", "../shaders/fragment_lighting_base.frag");
    baseShader.use();
    ...
    cameraPos = glm::vec3(0.0f, 1.2f, -3.0f);

    view = lookAt(cameraPos, worldOriginPos, glm::vec3(0.0f, 1.0f, 0.0f));
